...
def generate_html_report(data: ReportData, report_mode: str = "shield") -> str:
    """Render the full HTML report. report_mode: 'shield' (simple) or 'archer' (technical)."""
    is_shield = report_mode != "archer"
    ...
    if data.findings:
        for f in sorted(data.findings, key=lambda x: {"critical":0,"high":1,"medium":2,"low":3,"info":4}.get(x["severity"],5)):
            ...
            if is_shield:
                ...
                <tr>
                    ...
                    <td style="font-size:12px;">{_esc(f['description'])}</td>
                </tr>"""
            else:
                findings_html += f"""
                <tr>
        ...
        cols = 3 if is_shield else 6
        findings_html = f'<tr><td colspan="{cols}" style="text-align:center;color:#666;padding:24px;">No active security findings. ✓</td></tr>'
...
<title>Artemis {'Shield' if is_shield else 'Archer'} Security Report — {_esc(data.hostname)} — {data.generated_at}</title>
<style>
...
<div class="report">
    ...
    <div class="report-header">
        <div style="margin-bottom:12px;font-size:12px;letter-spacing:3px;text-transform:uppercase;color:#888;">
            {'🛡️ ARTEMIS SHIELD' if is_shield else '🏹 ARTEMIS ARCHER'} — {'Protection Report' if is_shield else 'Operations Report'}
        </div>
        <h1>{'[ SECURITY STATUS REPORT ]' if is_shield else '[ TECHNICAL SECURITY REPORT ]'}</h1>
        <div class="meta">
    ...
    <!-- Findings -->
    <div class="section">
        <h2>{'[ WHAT WE FOUND ]' if is_shield else '[ SECURITY FINDINGS ]'}</h2>
        {'<p style="color:#aaa;font-size:14px;margin-bottom:16px;">Here are the security issues Artemis detected on your system, ordered by importance.</p>' if is_shield else ''}
        <table>
            <thead><tr>
                <th>Severity</th><th>Finding</th>{'<th>Category</th><th>MITRE</th>' if not is_shield else ''}
                <th>Details</th>{'<th>Confidence</th>' if not is_shield else ''}
            </tr></thead>
    ...
    <div class="footer">
        <p>Generated by Artemis {'Shield' if is_shield else 'Archer'} v3.0.0 — {'Protection Report' if is_shield else 'Operations Report'}</p>
...
async def generate_report(db, classifier=None, ai_provider=None,
    ...
    html = generate_html_report(data, report_mode=report_mode)
